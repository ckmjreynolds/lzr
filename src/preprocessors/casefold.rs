...
impl Transform for CaseFolding {
    ...
    fn inverse(&self, input: Vec<u8>) -> anyhow::Result<Vec<u8>> {
        inverse_framed(&input, "case-folding", |rest| {
...
mod tests {
    ...
    fn casefold_lowers_the_alphabet() {
        ...
        let folded = CaseFolding.forward(b"NASA Reads XML".to_vec());
