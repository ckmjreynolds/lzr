//! Phase-7 codec: cost-aware LZ + same Order-3 / Order-2 Content
//! literal models as Phase 6.
//!
//! Same models as `xml-lz-ord3`. The only architectural change is at
//! the encoder's Content match decision: instead of "take any
//! `MIN_MATCH`-clearing match," compute the actual bit cost of both
//! options at this position and pick the cheaper one.
//!
//! - `match_cost`  = AC bits for `(flag=1, bucket, bucket-relative
//!    bits, length-MIN_MATCH)` — covers `length` bytes.
//! - `literal_cost` = AC bits for `(flag=0, is_letter, letter or
//!    non-letter byte)` — covers 1 byte.
//!
//! Take match iff `match_cost / length < literal_cost`. This lets
//! the per-mode literal models compete with LZ on a fair amortized
//! basis. Long matches always win; short marginal matches sometimes
//! lose to a predictable letter, freeing those bytes to be modeled
//! by Order-3 letter context.
//!
//! Decoder is unchanged in spirit from Phase 6 — it just reads
//! flags and decodes accordingly. The encoder's choice is fully
//! captured in the bit stream.
            ...
            while i < buf.len() {
                ...
                match mode {
                    Mode::Content => {
                        ...
                        // Cost-aware refinement on top of lazy parse. If lazy
                        // parse says "take match," compute the actual AC cost
                        // of the match (amortized per matched byte) vs the
                        // literal cost for the current byte; pick whichever is
                        // cheaper per byte. Letting the per-mode model "earn"
                        // the right to encode lets Order-3 letter context
                        // compete with marginal short matches.
                        let take_match = if lazy_take {
                            let (offset, length) = m_here.unwrap();
                            let match_cost = match_cost_bits(&models, offset, length);
                            let literal_cost = literal_cost_bits(&models, buf[i], &content_state);
...
/// Bit cost to encode `sym` against the AC's CDF gap. `gap == TOTAL`
/// gives 0 bits (perfectly predicted); `gap == 1` gives 16 bits
/// (least probable). Computed in f64 — only used by the encoder's
/// cost comparison, not on the decode path.
fn cost_bits(cdf: &[u32], sym: usize) -> f64 {
    let gap = (cdf[sym + 1] - cdf[sym]).max(1);
    (f64::from(crate::ac::TOTAL) / f64::from(gap)).log2()
}

/// Total AC bits to emit an LZ match record at `(offset, length)`
/// given the current model state. Includes the flag, bucket, raw
/// bucket-relative bits, and length token.
fn match_cost_bits(models: &Models, offset: u32, length: u32) -> f64 {
    ...
    cost_bits(&flag_cdf, 1)
        ...
        + f64::from(bucket) // bucket-relative bits are emitted raw (uniform CDFs are exact).
        + cost_bits(&length_cdf, len_token)
}

/// Total AC bits to emit a literal Content byte under the current
/// model state. Mirrors the literal branch's actual emission order:
/// `flag(0)`, `is_letter`, then either letter or non-letter byte.
fn literal_cost_bits(models: &Models, byte: u8, content_state: &ContentState) -> f64 {
    let mut flag_cdf = [0u32; 3];
    models.lz_flag.cdf_to(&mut flag_cdf);
    let mut is_letter_cdf = [0u32; 3];
    let ctx_byte = content_state.last_byte as usize;
    ...
    if is_letter_val == 1 {
        ...
        models
            .letter
            .cdf_to(content_state.letter_ctx(), &mut letter_cdf);
        bits += cost_bits(&letter_cdf, letter_idx);
    } else {
        let mut byte_cdf = [0u32; 257];
        models
            .nonletter
            .cdf_to(content_state.nonletter_ctx(), &mut byte_cdf);
