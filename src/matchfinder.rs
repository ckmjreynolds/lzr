...
// /// Capped at `u16::MAX` so the distance fits in [`Match::distance`].
// const LONG_CAPACITY: usize = u16::MAX as usize;
...
//         let mut len = prefix;
//         if reverse {
//             // In the decoder's circular window, a reverse copy writes forward
//             // and reads backward. If dist is close to WINDOW_SIZE, the write
//             // and read regions alias, corrupting later reads. Safe when the
//             // circular gap ≥ 2·len − 1, giving max len = (W + 1 − dist) / 2.
//             let rev_max = max_len.min((WINDOW_SIZE + 1 - dist) / 2);
//             if prefix > rev_max {
//                 continue;
//             }
//             while len < rev_max && len <= cand && buf[pos + len] == buf[cand - len] {
//                 len += 1;
//             }
//         } else {
