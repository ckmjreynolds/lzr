//! Order-0 byte model: a direct `StateMap` over the 8-bit bit-tree.

use super::statemap::StateMap;
use super::{Context, SYMBOL_BITS, TokenModel};

/// Predicts each bit purely from its position in the current byte's bit-tree (the
/// partial-symbol node `c0`), independent of prior bytes. A depth-8 tree has
/// `2^8 - 1` internal nodes, so a `2^8`-slot `StateMap` indexed by `c0` covers
/// every node.
#[derive(Debug)]
