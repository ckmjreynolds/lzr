...
/// Adaptive logistic mixer: one weight set per bit position over `n` stretched
/// model inputs. `mix` forms the weighted logit sum (16.16 fixed point) and
/// squashes it to a 12-bit probability; `update` nudges the selected weight set
...
impl Mixer {
    /// New mixer over `n` model inputs, with `bit_positions` independent weight
    ...
    /// Mix the `stretched` model outputs into a 12-bit probability using the
