//! Phase-8 codec: XML schema + LZ77 + PPM-C for Content literals.
//!
//! Same overall shape as `xml-lz-word` (lazy LZ, `is_letter` flag,
//! letter/non-letter routing), but the letter and non-letter
//! predictors are PPM-C with multi-order escape instead of dense
//! Order-1 / Order-3 tables. Letters run at order 5 (alphabet 52);
//! non-letters at order 4 (alphabet 256). The escape mechanism lets
//! sparse high-order contexts contribute predictions when seen and
//! cleanly fall back to lower orders when not.
//!
//! Decoder mirrors the encode-side escape chain exactly via the
//! shared `PpmC` `encode` / `decode` methods.
...
pub(crate) struct XmlLzPpmcCodec;

// Empirical sweep on the enwik8 panel (Phase 8 commit message):
//   order 2/1: 2.626  |  order 3/2: 2.617 (best)
//   order 4/3: 2.639  |  order 5/4: 2.675
// Higher orders lose because PPM-C without exclusions pays escape
// bits at each fallback step without the exclusion mechanism PPM-D
// uses to keep lower-order CDFs tight. Settling at 3/2 for now.
const LETTER_PPM_ORDER: usize = 3;
const NONLETTER_PPM_ORDER: usize = 2;
...
struct Models {
    ...
    /// Order-5 PPM-C over the letter stream (skips non-letters in
    /// the model's history, since `observe` is only called for
    /// letter bytes).
    letter: PpmC,
    /// Order-4 PPM-C over Content non-letter bytes. Sees only
    /// non-letter Content bytes in its history.
    nonletter: PpmC,
}
...
impl Models {
    fn new() -> Self {
        Self {
            ...
            letter: PpmC::new(u32::try_from(N_LETTERS).unwrap(), LETTER_PPM_ORDER),
            nonletter: PpmC::new(256, NONLETTER_PPM_ORDER),
        }
    }
}

/// Running state for the Content literal codec: just the last byte,
/// used as the `is_letter` flag's context. The letter and non-letter
/// `PpmC` models track their own history internally.
#[derive(Clone, Copy, Debug, Default)]
struct ContentState {
    last_byte: u8,
}
...
impl Codec for XmlLzPpmcCodec {
    ...
    fn encode_window(&self, warm: &[u8], measure: &[u8]) -> Result<(Vec<u8>, Decomposition)> {
        ...
        // letter_cdf is unused — PpmC builds CDFs internally.
        {
                    ...
                    Mode::Content => {
                        ...
                        if take_match {
                            ...
                            for &b in &buf[i..i + len_us] {
                                ...
                                // Match bytes don't update count tables, but
                                // they DO slide model history so future
                                // literal predictions see the actual byte
                                // stream as context.
                                if let Some(idx) = letter_to_idx(b) {
                                    models.letter.slide_history(u16::from(idx));
                                }
                                models.nonletter.slide_history(u16::from(b));
                            }
                            i += len_us;
                        } else {
                            ...
                            if is_letter_val == 1 {
                                let letter_idx = u16::from(
                                    letter_to_idx(byte).expect("just classified as letter"),
                                );
                                models.letter.encode(&mut enc, letter_idx);
                                // Letter also enters the non-letter PpmC's
                                // history (its context is cross-class) but
                                // doesn't update its count tables.
                                models.nonletter.slide_history(u16::from(byte));
                            } else {
                                // Non-letter literal — emit via non-letter
                                // PpmC. Letter PpmC's history skips
                                // non-letters, so no slide there.
                                models.nonletter.encode(&mut enc, u16::from(byte));
                            }
                ...
                Mode::Content => {
                    ...
                    if flag == 1 {
                        ...
                        let b = if is_letter_val == 1 {
                            let letter_idx = models.letter.decode(&mut dec)?;
                            let byte = idx_to_letter(letter_idx as usize);
                            models.nonletter.slide_history(u16::from(byte));
                            byte
                        } else {
                            let sym = models.nonletter.decode(&mut dec)?;
    ...
    while i < warm.len() {
        ...
        match mode {
            Mode::Content => {
                ...
                if is_letter_val == 1 {
                    let letter_idx =
                        u16::from(letter_to_idx(byte).expect("just classified as letter"));
                    models.letter.observe(letter_idx);
                    models.nonletter.slide_history(u16::from(byte));
                } else {
                    models.nonletter.observe(u16::from(byte));
                }
                classifier.advance(byte);
                content_state.advance(byte);
                i += 1;
