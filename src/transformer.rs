...
mod tests {
    ...
    fn pytorch_logit_parity_small_42_step2000() {
        ...
        let logits_path =
            "/Users/creynolds/Programming/lzr-neural/ckpts/small_42_step2000.logits.bin";
        if !std::path::Path::new(lzrn_path).exists() || !std::path::Path::new(logits_path).exists()
        {
            eprintln!(
                "parity artifacts missing — skipping (this is OK on machines without lzr-neural built)"
            );
            return;
        }
        ...
        eprintln!(
            "logit parity: t={t} vocab={vocab}  max_abs={max_abs:.6}  mean_abs={mean_abs:.6}"
        );
        // Threshold: 1e-3 max abs is loose enough to absorb f32
        // accumulation order differences between `PyTorch` (MPS) and
        // our scalar Rust matmul; tight enough to catch any real
        // arithmetic mistake (e.g., wrong layer, missed residual, etc.)
        assert!(
            max_abs < 1e-3,
            "logit divergence too large: max_abs={max_abs}"
        );
    }
