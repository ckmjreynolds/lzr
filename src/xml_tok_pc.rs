//! Phase-17 codec: page-local cache layered on `xml-tok`.
//!
//! Adds a per-page ring-buffer cache of 4096 dictionary ids, reset
//! on each `<page>` boundary detected by a 6-byte sliding window
//! over the (decoded) byte stream — same logic on encoder and
//! decoder. For every Content hit-token the encoder first emits a
//! `cache_flag` bit conditioned on `prev_id`. On a cache hit it
//! then emits a 12-bit cache position via its own adaptive bit
//! predictor (collapses well below 12 bits because MRU positions
//! dominate). On a cache miss the standard 18-bit `dict_id` emission
//! runs unchanged.
//!
//! The Phase-0 recon (`recon.rs`, run on enwik9) showed the
//! top-1000 tokens have mean avg-occurrences-per-page = 2.89, with
//! mid-rank outliers like "td" at avg 76. The cache amortizes the
//! bit-predictor's slow per-context warm-up across page-concentrated
//! rare tokens — Path A on those costs 12–18 bits while a cache hit
//! costs ~9 bits + a small `cache_flag` overhead. Hot tokens (rank
//! 1–100) keep paying near-Shannon Path A; the flag predictor learns
//! to emit `cache_flag = 0` cheaply for them.
//!
//! Original xml-tok docs below.
//!
...
/// Page-cache capacity in dictionary-id slots. Swept on the enwik8
/// full panel; gain is monotone up to ~128 slots (-0.042 bpb) then
/// plateaus. 128 means 7 bits raw position cost; combined with the
/// `cache_pos_bit` predictor's residual learning, the effective per-
/// hit cost lands around 5–6 bits — well below the bit predictor's
/// cost on the rare-but-page-repeated tokens this codec targets.
const PAGE_CACHE_LOG2: u32 = 7;
const PAGE_CACHE_SIZE: usize = 1 << PAGE_CACHE_LOG2;
const PAGE_CACHE_POS_BITS: u32 = PAGE_CACHE_LOG2;

/// Bit-predictor table size for the `cache_flag` model. Context is
/// `(prev_id)` only — small space, modest table.
const CACHE_FLAG_BIT_K: u32 = 18;
/// Bit-predictor table size for `cache_position`. Context is
/// `(prefix, bit_pos)` only — NOT conditioned on `prev_id` because
/// the encoded value is recency-relative and its high-bit-zero
/// skew is `prev_id`-independent. With a 2 ^ 12 ring buffer and 12
/// bit positions the joint context space is ~50 K; a 2 ^ 16 table
/// gives ~10x headroom for collisions.
const CACHE_POS_BIT_K: u32 = 16;

/// 6-byte sliding-window match for `<page>`. Encoder and decoder
/// both maintain the same tail register and reset their `PageCache`
/// when the tail equals `PAGE_OPEN`. Detection runs on every byte
/// pushed to `buf` — including bytes emitted by an LZ match — so
/// it's invariant to the codec's encoding path.
const PAGE_OPEN_PATTERN: u64 = ((b'<' as u64) << 40)
    | ((b'p' as u64) << 32)
    | ((b'a' as u64) << 24)
    | ((b'g' as u64) << 16)
    | ((b'e' as u64) << 8)
    | (b'>' as u64);
const PAGE_TAIL_MASK: u64 = (1u64 << 48) - 1;

/// Ring-buffer cache of dict ids seen in the current page. New ids
/// insert at `next`; if `lookup` already has them, the existing
/// slot is left alone (no LRU promote — simpler and the bit
/// predictor on position learns recency anyway). When the ring
/// fills, the slot at `next` is overwritten and its old id is
/// removed from `lookup`. Reset on every `<page>`.
#[derive(Debug)]
struct PageCache {
    ring: Vec<u32>,
...
impl PageCache {
    fn new() -> Self {
        Self {
            ring: vec![0u32; PAGE_CACHE_SIZE],
            lookup: HashMap::with_capacity(PAGE_CACHE_SIZE),
            next: 0,
            filled: 0,
        }
    }
    ...
    fn push(&mut self, id: u32) {
        ...
        if self.filled == PAGE_CACHE_SIZE {
            let evicted = self.ring[self.next];
            self.lookup.remove(&evicted);
        } else {
            self.filled += 1;
        }
        let pos = u32::try_from(self.next).expect("PAGE_CACHE_SIZE fits u32");
        self.ring[self.next] = id;
        self.lookup.insert(id, pos);
        self.next = (self.next + 1) % PAGE_CACHE_SIZE;
    }

    fn at(&self, pos: u32) -> u32 {
        self.ring[pos as usize]
    }
}

/// Per-byte page-detection update. Both encoder and decoder advance
/// this on every byte pushed to `buf` (including the warm prefix
/// and any LZ-matched bytes), so cache resets fire at exactly the
/// same byte positions on both sides.
const fn page_check(tail: &mut u64, byte: u8) -> bool {
    *tail = ((*tail << 8) | byte as u64) & PAGE_TAIL_MASK;
    *tail == PAGE_OPEN_PATTERN
}

/// Advance the XML classifier and the page-detector tail by one
/// byte, and reset the page cache if a `<page>` boundary was just
/// closed. Encoder and decoder both go through this helper for
/// every Content/Tag/Attr byte they commit, so the cache state
/// stays in lock-step across the two sides.
fn advance_byte(
    ...
    if page_check(page_tail, byte) {
        page_cache.reset();
    }
}

#[derive(Clone, Copy, Debug, Default)]
pub(crate) struct XmlTokPcCodec;
...
struct Models {
    ...
    /// Phase 17: bit-level predictor for the page-cache flag.
    /// Context = `(prev_id)`. Learns when the cache pays off — for
    /// hot tokens the predictor collapses to "always 0" cheaply, so
    /// the flag costs a fraction of a bit. For rare-but-recently-seen
    /// tokens it learns "often 1".
    cache_flag_bit: BitPredictor,
    /// Bit-level predictor for the 12-bit `cache_position` value
    /// emitted on a cache hit. Same `(prev_id, prefix, bit_pos)`
    /// shape as `token_id_bit`. Recency in the ring buffer means
    /// MRU positions cluster around `cache.next`, so the high bits
    /// are highly predictable.
    cache_pos_bit: BitPredictor,
}
...
impl Models {
    fn new() -> Self {
        Self {
            ...
            cache_flag_bit: BitPredictor::new(CACHE_FLAG_BIT_K),
            cache_pos_bit: BitPredictor::new(CACHE_POS_BIT_K),
        }
    }
}
...
struct ComponentBits {
    ...
    tag: u64,
    attr: u64,
    /// Phase-17 split: cache-flag bits + cache-position bits when
    /// the flag fired. `token_hit` now excludes the cache-path bits
    /// so the decomposition shows where Phase-17 spent vs saved.
    cache_flag: u64,
    cache_pos: u64,
}

impl Codec for XmlTokPcCodec {
    ...
    fn encode_window(&self, warm: &[u8], measure: &[u8]) -> Result<(Vec<u8>, Decomposition)> {
            ...
            while i < buf.len() {
                ...
                match mode {
                    Mode::Content => {
                        ...
                        if let Some((offset, length)) = lz_match {
                            ...
                            for &b in &buf[i..new_i] {
                                advance_byte(&mut classifier, &mut page_tail, &mut page_cache, b);
                            }
                            // Cache push happens AFTER advance so a
                            // boundary inside this LZ payload resets
                            // the cache before the matched ids land
                            // — but since LZ payload is a sequence of
                            // word/separator tokens, `<page>` would
                            // not span them; do it here regardless.
                            for lh in lookahead.iter().take(length_us) {
                                page_cache.push(lh.id);
                            }
                            i = new_i;
                    ...
                    Mode::AttrValue => {
                        ...
                        advance_byte(&mut classifier, &mut page_tail, &mut page_cache, buf[i]);
                        i += 1;
        ...
        decomp.add("cache_flag_ac", comp.cache_flag);
        decomp.add("cache_pos_ac", comp.cache_pos);
        ...
        while buf.len() - warm_end < measure_len {
            ...
            match mode {
                Mode::Content => {
                    ...
                    let (token_bytes, new_prev) = decode_token(
                        &mut dec,
                        &mut models,
                        &mut dict,
                        &page_cache,
...
///
/// Phase-17: on a dict hit, consult the page cache. If the id is
/// present, emit `cache_flag = 1` and the cache position. Else emit
/// `cache_flag = 0` and the standard 18-bit `dict_id`. The flag
/// predictor learns when each path is profitable — for hot tokens
/// it converges on "always 0" (cheap), for page-concentrated rare
/// tokens it learns "often 1".
...
fn encode_token(
    ...
    if let Some(id) = hit_id {
        let cache_pos = page_cache.get(id);
        let flag_ctx = cache_flag_ctx(prev_id);
        let p_zero = models.cache_flag_bit.predict_p_zero(flag_ctx);
        let flag_cdf = [0u32, p_zero, TOTAL];
        let flag_sym = u32::from(cache_pos.is_some());
        let cf_before = enc.bits_written();
        enc.encode(&flag_cdf, flag_sym as usize);
        models.cache_flag_bit.observe(flag_ctx, flag_sym);
        comp.cache_flag += enc.bits_written() - cf_before;

        if let Some(pos) = cache_pos {
            let pos_bits_before = enc.bits_written();
            encode_cache_pos(enc, models, pos, page_cache.next);
            comp.cache_pos += enc.bits_written() - pos_bits_before;
        } else {
            let token_hit_before = enc.bits_written();
            encode_dict_id(enc, models, prev_id, id);
            comp.token_hit += enc.bits_written() - token_hit_before;
        }
...
fn decode_token(
    ...
    if oov_sym == 0 {
        ...
        let p_zero = models.cache_flag_bit.predict_p_zero(flag_ctx);
        let flag_cdf = [0u32, p_zero, TOTAL];
        let flag_sym = u32::try_from(dec.decode(&flag_cdf)?).expect("bit symbol fits u32");
        models.cache_flag_bit.observe(flag_ctx, flag_sym);

        let id = if flag_sym == 1 {
            let pos = decode_cache_pos(dec, models, page_cache.next)?;
            page_cache.at(pos)
        } else {
            decode_dict_id(dec, models, prev_id)?
        };
        let lower = dict.entry(id).lower.clone();
        let bytes = if class == TokenClass::Word {
...
/// Context hash for the `cache_flag` bit predictor. The flag is one
/// bit per Content hit-token, so we don't need a `bit_pos` or
/// `prefix` — just the previous token's id. `None` (start of a
/// Content run or after an OOV-overflow token) gets a distinct
/// sentinel via the same `u64::MAX` trick `id_bit_ctx` uses.
fn cache_flag_ctx(prev_id: Option<u32>) -> u64 {
    let prev_field = prev_id.map_or(u64::MAX, u64::from);
    fnv_mix(FNV_OFFSET, prev_field)
}

/// Convert an absolute ring slot to a recency-relative one. The MRU
/// slot — the one filled by the most-recent `push` — has rel = 0;
/// older entries have higher rel up to `PAGE_CACHE_SIZE - 1`.
fn slot_to_rel(slot: u32, cache_next: usize) -> u32 {
    let next = u32::try_from(cache_next).expect("cache_next < PAGE_CACHE_SIZE fits u32");
    let cap = u32::try_from(PAGE_CACHE_SIZE).expect("PAGE_CACHE_SIZE fits u32");
    (next + cap - 1 - slot) % cap
}

/// Inverse of `slot_to_rel`.
fn rel_to_slot(rel: u32, cache_next: usize) -> u32 {
    let next = u32::try_from(cache_next).expect("cache_next < PAGE_CACHE_SIZE fits u32");
    let cap = u32::try_from(PAGE_CACHE_SIZE).expect("PAGE_CACHE_SIZE fits u32");
    (next + cap - 1 - rel) % cap
}

/// Hash `(prefix, bit_pos)` into the `cache_pos_bit` predictor's
/// context space. Deliberately omits `prev_id` — see the
/// `CACHE_POS_BIT_K` comment.
fn cache_pos_ctx(prefix: u32, bit_pos: u32) -> u64 {
    let h = fnv_mix(FNV_OFFSET, u64::from(prefix));
    fnv_mix(h, u64::from(bit_pos))
}

/// Encode a cache position as the **recency-relative** rank in the
/// ring (`0` = most recent insertion, growing toward older). The
/// recency rank's distribution is skewed toward small values — a
/// page-cached token typically reappears shortly after a recent
...
fn encode_cache_pos(
    ...
    for bit_pos in (0..PAGE_CACHE_POS_BITS).rev() {
        let bit = (rel >> bit_pos) & 1;
        let ctx = cache_pos_ctx(prefix, bit_pos);
        let p_zero = models.cache_pos_bit.predict_p_zero(ctx);
        let cdf = [0u32, p_zero, TOTAL];
        enc.encode(&cdf, bit as usize);
        models.cache_pos_bit.observe(ctx, bit);
        prefix = (prefix << 1) | bit;
    }
}
...
fn decode_cache_pos(
    ...
    Ok(rel_to_slot(prefix, cache_next))
}
    ...
    while i < warm.len() {
        ...
        match mode {
            Mode::Content => {
                ...
                for &b in token {
                    advance_byte(classifier, page_tail, page_cache, b);
                }
                i = end;
                last_class_ctx = match class {
                    TokenClass::Word => CLASS_CTX_WORD,
            ...
            Mode::AttrValue => {
                ...
                advance_byte(classifier, page_tail, page_cache, warm[i]);
                i += 1;
                last_class_ctx = CLASS_CTX_NONE;
                prev_id = None;
            }
...
fn observe_token(
    ...
    if let Some(id) = hit_id {
        // Mirror the encode-side cache-flag/path observation so the
        // cache_flag_bit predictor and (when used) cache_pos_bit
        // predictor arrive at measure time with calibrated priors.
        let cache_pos = page_cache.get(id);
        let flag_ctx = cache_flag_ctx(prev_id);
        let flag_sym = u32::from(cache_pos.is_some());
        models.cache_flag_bit.observe(flag_ctx, flag_sym);
        if let Some(pos) = cache_pos {
            let rel = slot_to_rel(pos, page_cache.next);
            let mut prefix: u32 = 0;
            for bit_pos in (0..PAGE_CACHE_POS_BITS).rev() {
                let bit = (rel >> bit_pos) & 1;
                let ctx = cache_pos_ctx(prefix, bit_pos);
