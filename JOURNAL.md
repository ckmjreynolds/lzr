...
SHIPPED: a 2-head stack `{order-1 prev-byte, word}` at lr=4, `HEAD_BITS=16` (two ~64 MB tables; enwik9 RSS ~8.18→~8.3 GB, under cap). Full-stack 10 MB marginal −0.0154 over no-head (vs the per-bit-tree head's −0.0053 and order-1-alone's −0.0126). Byte-exact round-trip (`roundtrip_enwik8_slice`, `nmix_roundtrip`); build.sh green; L(D) unchanged (0.01225). `LZR_NO_HEAD` drops the stack; probe `diversity_lab` gains `LZR_HEAD2`/`LZR_HEAD2W`/`LZR_HEAD2CTXB` for a second head. (Methodology note: an earlier two-NET probe read −0.0163 for the same pair, inflated ~0.0009 by a redundant duplicate frozen input; the one-net stack's −0.0154 is the production-faithful number.) Next: more decorrelated head contexts (sparse/skip, higher order) — each ~free if it adds signal. enwik9 not run (CDR); the stack's warming/holding behavior projects a likely new project best over 1.1812.
