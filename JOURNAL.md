...
## 2026-07-02 → 2026-07-03 — enwik9 CONFIRMED: net 1.1303 (L(C) 1.1173 + L(D) 0.01305), a −0.0113 NEW PROJECT BEST — the match-order lever amplified ~2× past its projection

The enwik9 encode of the overnight session's three ships (key-12/16 match models, reorder RAM diet, 18-bit head tables + key-4@25) finished: 1,000,000,000 → 139,661,944 B = **L(C) 1.1173**, plus L(D) 0.013048 (binary 815,504 B), **net 1.1303** — −0.0113 over the standing 1.1416, beating the −0.003..−0.005-per-lever projection band (1.136–1.139) by ~0.006. Encode wall 44,162 s (12.27 h), ~2% over the prior run despite the added models — the +20% seen on the full-enwik8 gates did not materialize at enwik9 pacing. CDR approved encode-only; the byte-exact round-trip is guarded by the build.sh integration test and the 07-02 full enwik9 round-trip of the prior stack, but the decode gate on THIS archive is owed before submission-final.

The upside came from coverage amplification beyond the projection's assumption: key-12 coverage 25.8% (enwik8) → **44.4%** (enwik9) and key-16 13.6% → **29.9%**, versus key-8's 57.3% → 75.2%. The higher orders benefit disproportionately from enwik9's deep cross-article redundancy, exactly the regime v7's order-scaling exploited — and this time inside the 10 GB model budget. The key-4@25 shrink is validated at scale: 13.5% occupancy, 0.3% collisions over 728M writes — cardinality-bounded as predicted.

Two follow-ups surfaced. (1) **The key-12/16 finders are SATURATED at enwik9** — occupancy 94.1%/97.5%, collision rates 40.6%/54.1% of writes — so they are losing acquisitions to overwrites; larger tables (bits 28, +768 MB each) are now the most concrete next bpb lever, RAM permitting. (2) **Instantaneous RSS ran ~12–13 GB** on the 38 GB dev machine — well above the ~8.6 GB live-table ledger — because the reorder pass's ~243k freed per-article maps sit as dead-but-resident allocator-arena pages that macOS reclaims only lazily. The live footprint matches the ledger (RSS declined from 13.8 as pages were reclaimed, and the diet's frees are real), but a submission-grade RAM claim under the 10 GB judging cap needs a proper live-footprint measurement and possibly an arena-conscious reorder (e.g., running the ordering in a bump-arena or freeing via a scoped sub-process) — flagged as pre-submission engineering.
...
## 2026-07-02 — net capacity × the warming-head channel is a NEGATIVE: H=512 is unreachable (a dead-embedding width threshold in the trainer), and H=384's real slice gain erodes away at scale — the heads do not convert frozen capacity into a warming gain

CDR approved testing the one net-scaling axis the "frozen-net lever is spent" verdict (06-25/26) never covered: hidden width H against the WARMING-HEAD channel. The rationale: the quality and K sweeps predate the heads; the heads read the frozen embedding and held their full marginal at enwik9 (06-27) where the frozen logit eroded ~2×, so a richer embedding could in principle pay through the heads even where the net's own logit does not. H moved from a const to `LZR_H` (the blob header already carries H, so the CPU loader is unchanged; commit 65fa03a), and the training data was re-dumped from the CURRENT pipeline — reorder included — at 72,620,798 B, byte-identical to the recorded deterministic figure, incidentally re-confirming cross-process pipeline determinism.

The first finding is a TRAINER wall, not a capacity answer: **H=512 falls into a dead-embedding equilibrium** — |emb| decays monotonically to 0 within the first ~2K steps while the output biases capture the unigram (loss pinned ~4.1 nats for the remaining 450K steps) — at lr 1e-3, at lr 5e-4, and even through a 10K-step linear warmup (|emb| 6605→5296→2774→855→120→6→0 across the warmup's tiny LRs; `LZR_WARMUP` knob added, commit 532a815). So the failure is not LR magnitude: past a width threshold the gradient dynamics starve the representation path once the biases own the easy unigram bits. The e2e confirms totality: the H512 net in the shipped stack gives 1.5608 at 20 MB vs the 1.4795 base — the mixer actively routing around a noise input. Controls isolate the trigger: **H=256 and H=384 train perfectly on the identical data/recipe** (~1.9–2.1 nats, |emb| stable), so the stability threshold is in (384, 512]. This rhymes with the 06-24 "collapse to marginal" pathology (then a burn flatten-backward bug, fixed) but is a distinct mechanism reproduced by width alone. Candidate fixes for any future attempt: decoupled/raised embedding LR, embedding-row normalization, width-aware output init, or scaling E instead of H.

The second finding answers the hypothesis, negatively. **H=384** (500,352 params, q8 502,300 B, L(D) +0.0026 over the shipped 337,948 B) earns a real **−0.0040 at 20 MB** (1.4755 vs 1.4795) — but the full-enwik8 gate erodes it to **−0.0012** (1.3856 vs 1.3868), the classic frozen-contributor signature, NOT the hold-or-grow signature of head-carried gains. Projecting the usual further enwik8→enwik9 erosion (~2×) gives ~−0.0006 L(C) against +0.0026 L(D): clearly net-negative, and the wider forward also costs ~+22% encode wall (5,902 s vs ~4,837 s full enwik8). Verdict: the warming heads do NOT rescue frozen-net capacity scaling — the extra embedding richness is absorbed by the warming online stack at scale exactly like frozen-logit quality was. The "frozen-net lever is spent" conclusion now covers the capacity axis with the head channel included. Shipped net stays; asset restored.

A useful byproduct closed a smaller open question: **retraining H=256 on the reordered-pipeline dump is an exact e2e tie with the shipped net** (1.4796 vs 1.4795 at 20 MB, same blob size) — confirming at the full stack what the 06-28 analysis predicted from the K=32 window's article-locality: there is no "retrain on reordered data" win to collect.

## 2026-07-02 — warming-head tables were collision-starved: every head's node hashes its context WITH the bit-tree position, so 16-bit tables saturate at order-1; 18-bit tables gate at −0.0016 (1.3884 → 1.3868), funded by a neutral key-4 finder shrink

Second finding of the overnight session (below). A HEAD_BITS probe motivated by cardinality arithmetic: a head's table node is `hash(context, bit-tree position)`, and the ×256 tree-position factor means even the order-1 head addresses 256×256 distinct nodes — exactly a 16-bit table — while word, order-2, sparse, and the match-derived heads overflow 16 bits outright. Only `num` fits. The 20 MB sweep confirms starvation and shows no knee: all-heads at 17 bits −0.0007, at 18 bits −0.0012 over the 16-bit base (1.4808), with the selective high-cardinality-only variant (word/order-2/sparse pair) capturing just −0.0004 — the gain is spread across every head, as the tree-position arithmetic predicts. The full-enwik8 gate PASSED at **−0.0016 (1.3884 → 1.3868)**, GROWING from the 20 MB −0.0012 — collision pressure grows with data, so the enwik9 marginal should hold or grow. Selective 19-bit tables on the two biggest heads (order-1, word) add nothing further (+0.0002 at 20 MB); the knee is 18 at this scale.

The RAM funding is its own small finding: the key-4 match finder's 4-byte contexts are cardinality-bounded (~2M distinct at full enwik8, occupancy 2.8% of a 2^25 table at 20 MB), so shrinking it 27→25 bits is bpb-NEUTRAL by exact deterministic A/B while reclaiming 576 MB — table size only matters for unbounded key spaces. Ledger: 18-bit heads cost 256 MB/head (11 heads ≈ 2.8 GB), net +2.2 GB against the shrink; enwik9 coding RSS ~8.6 GB (verified live at 8.63 GB during the gate encode), inside the 8.5–8.7 GB precedent under the 10 GB cap. Shipped (commit a6556b4): `HEAD_BITS` 16→18, key-4 default bits 25 (`LZR_M4BITS` sweep knob kept); binary 815,504 B, L(D) 0.01305 unchanged.

Dial re-tunes on the grown stack all came back NULL, confirming the tuned surface is stable: mixer `LR_SHIFT` 12/13/14 = 1.4822/**1.4795**/1.4804 (13 stays), `HEAD_LR` 6 at the sparser 18-bit tables +0.0020 (4 stays), `NMIX_H` 128 only −0.0002 over 96 (below the ship bar at +33% mixer compute). `diversity_lab` gained per-token table-bits overrides (`CTX:LR:BITS`) and a known-base skip (`LZR_HEADS_BASE_BPB`).

## 2026-07-01 → 2026-07-02 — match order-scaling ports to v9: key-12/key-16 match models gate at −0.0029 (1.3913 → 1.3884) with a scale-GROWING marginal; the multi-candidate/verified half of v7's xmatch does NOT transfer

CDR set an autonomous overnight session (one experiment at a time — the enwik9 decode round-trip was running concurrently) with an explicit directive to weight fresh judgment over the journal's backlog. The chosen main line was the one v7 deterministic lever never ported to v9: match ORDER SCALING, v7's biggest win (−0.0094), whose enwik9-shippability was v7's open problem. v7's xmatch bundled two mechanisms; they were separated and measured independently on the full shipped stack (`match_order_lab`, same-process marginals; cross-process reorder determinism also re-confirmed empirically — identical base bpb across separate processes at 8 and 20 MB, which makes single-encode gates against recorded figures sound, via the new `LZR_MBASE` override).

Mechanism 1, discrete higher-order keys, TRANSFERS and is shipped (commit 4cc1cc4). `MatchModel` grew a second rolling u64 (bytes 9..16 back) folded into the finder key, so keys up to 16 bytes work; `baseline_models()` appends key-12 and key-16 matches after the key-8/key-4 pair (the mixer match-selector and the heads' match-state deposits take the first match models, so both are untouched). The marginal GROWS monotonically with scale — key-12 −0.0009 (8 MB), key-12+16 −0.0017 (20 MB), **full-enwik8 gate −0.0029 (1.3913 → 1.3884)** — the coverage-driven signature (key-12/key-16 coverage 20.1%/11.0% at 20 MB rises to 25.8%/13.6% at full enwik8, and enwik9 should roughly double both, as key-8 goes 57.3% → 75.2%). Projected enwik9: −0.003 to −0.005. RAM +2×768 MB flat finders; binary +16 B; encode wall grew ~20% across the session's two ships (gate encodes 4,733–4,837 s vs the prior ~3,945 s full-enwik8) → enwik9 ~14.5 h/dir, inside the ~41 h budget. Whether the key-12/16 tables can shrink to 26 bits (768 MB back) is an enwik9-scale-only question — slice occupancy is too low to measure it.

Mechanism 2, multi-candidate verified acquisition, does NOT transfer — a clean negative. A `SetFinder` (4 ways/bucket, newest first) plus `verify_best`: every candidate's context is verified against real history (killing tag false-positives exactly), the longest verified context wins, and the match length is SEEDED from the verified length so a long-verified acquisition starts already trusted by the length-keyed StateMap — the continuous form of a higher-order key. As a REPLACEMENT for the primary key-8 flat finder it is NEUTRAL (+0.0001/+0.0001/−0.0000 at 8 MB across RAM-equal, RAM-half, and shallow-verify configs); as an APPENDED extra it earns −0.0010 alone at 20 MB but only −0.0001 on top of key-12+16 — it carries the same "a long context repeats here" information as the discrete orders, at worse RAM economics. v7's candidate-count lever was compensating for its weaker mixing; v9's StateMap+mixer already extract what candidate choice added there. Kept `#[cfg(test)]` with the lab (`LZR_MKEYS` `vK@B:VCAP` tokens, `LZR_MREPL`).

Methodological note for future labs: model ORDER in the models vec shifts 20 MB bpb by ~±0.0003 (key-12/16 appended after the net vs shipped before the indirect models), so marginals must be compared within one construction form; the gate used the appended form and the shipped form is the enwik9 arbiter.

## 2026-07-01 — reorder RAM diet: the enwik9 reorder peak drops ~9.3 → ~6 GB and the coding phase frees the raw input (−1 GB) — computation unchanged

The 07-01 enwik9 confirmation flagged the in-process reorder pass peaking ~9.3 GB, tight under the 10 GB judging cap. Three allocations were dead-but-held (commit 56de483): the 243k per-article TF `HashMap`s (~3 GB at enwik9) lived to the end of `similarity_order` though dead once the TF-IDF vectors are built — `tfs.into_iter()` now frees each map as its vector is built; the global DF map (~1 GB) lived through the greedy loop though dead once the inverted index exists — dropped explicitly; and `main`'s raw input (1 GB) was held across the entire multi-hour coding phase though dead after the pipeline forward — `encode()` now takes the input by value and drops it post-pipeline, so coding holds one stream copy, not two. Computation is unchanged (same iteration order, same sums; the canonical-sort determinism fix is untouched); reorder round-trips and the full suite stay green. The reclaimed headroom is what funds the session's two RAM-costing ships (the key-12/16 finders and the 18-bit head tables) at a projected ~8.6 GB enwik9 coding RSS.
...
The reorder fix is a submission-reproducibility fix, not a correctness one. `reorder.rs::similarity_order` claimed determinism but built its TF-IDF vectors from a `HashMap` iterated in per-process-random order, and the greedy chain's cosine score is an f32 sum — non-associative — so the accumulation order varied run to run, flipping near-ties and producing a different article permutation (and ±0.001 bpb) each process. Round-trip was never affected (the decoder restores order by id-sort, never replaying the permutation), but a Hutter submission must produce a byte-identical archive every build. The fix canonically sorts each block's TF-IDF vector by word hash before the norm and cosine sums, pinning the accumulation order. Verified: two separate processes now emit byte-identical full-enwik8 preprocessed output (72,620,798 B, `cmp` identical). The pinned order is one sample from the prior ±0.001 noise band — reproducible, not optimized; a deterministic 2-opt refinement of the order is a separate future lever.

The head triple — `num` (digit-field, exp 3, `HEAD_LR`), `m2c` (secondary-match-pred × prev, exp 6, `HEAD_MATCH_LR`), and a sparse bytes-back {1,4} head — is now in `models()`, so `default = submission` carries it (the mixer/nmix auto-size to the three extra inputs). RAM grows ~190 MB (three HEAD_BITS-16 tables; enwik9 RSS ~8.5 → ~8.7 GB, under the 10 GB cap; the `num` head's digit-field context has <256 values and could later use a far smaller table). The enwik9 encode is running to establish the new reproducible standing best — the prior 1.1446 used the now-removed random reorder, so this run re-baselines and confirms the −0.0022 head gain at 10× scale in one shot; the number lands as a follow-up. If the heads unexpectedly fail to carry, the ship is a one-line revert.

## 2026-07-01 — the `num,m2c,s9` head triple PASSES its full-enwik8 gate at −0.0022, and the marginal GROWS with scale (heads warm with data)

Follow-up to the overnight diversity sweep below. The sweep's keeper — three decorrelated warming heads (`num` digit-field, `m2c` secondary-match-pred × prev at lr 2, `s9` sparse bytes-back {1,4}) over the shipped 8-head stack — was gated on all of enwik8 through `diversity_lab` (same process, one reorder permutation, so the difference is exact; no shipped code changed). The base spec reproduces the shipped stack exactly: its full-enwik8 bpb is 1.3935, matching the recorded shipped figure, so the lab gate is the real gate. Result: base 1.3935 → triple 1.3913, marginal −0.0022. The marginal grows monotonically with scale — −0.0018 (8 MB) → −0.0018 (20 MB) → −0.0020 (40 MB) → −0.0022 (full enwik8) — the signature of online heads warming with more data (unlike a frozen contributor, whose slice marginal is an upper bound that erodes at scale). So it should hold, and plausibly grow, toward the enwik9 target: roughly 1.1446 → ~1.1424 net if it carries, at L(D)≈0.

The lever is a gate-clean, small, shippable improvement — the first movement on the standing best since the reorder ship — but the ship decision is still CDR's: it needs an enwik9 confirmation (owed, ~10 h, not run tonight without approval) and a RAM-headroom check (+~190 MB for three HEAD_BITS-16 tables on top of the ~8.5 GB enwik9 RSS; the `num` head's digit-field context has fewer than 256 distinct values and can use a much smaller table). The three `push_head_*` calls to add to `models()` are written up in the session's proposal; they are not yet made. Do not add the near-miss heads (`mm` overlaps `m2c`; `s10` {2,4} hurts; a fourth sparse `s17` was only −0.0002 at 20 MB).

## 2026-07-01 — overnight warming-head diversity sweep: head diversity is NOT mined at 8 — a decorrelated `num,m2c,s9` triple adds −0.0018 over the shipped stack (scale-stable 8→20 MB); plus reorder is cross-process non-deterministic (measurement noise, not a bug)

CDR set an autonomous overnight session on the journal's own diagnosis — that the gap is a base-model-diversity problem, not a mixing-machinery one (2026-06-24) — after confirming from the journal that the recurrent-mixer "M2" was already settled negative (redundant with the arm). In this architecture base-model diversity is best explored as warming-head keyings: a head is the L(D)≈0, full-stack-measured, shippable form of a new decorrelated model (an online linear readout over the frozen net's embedding, keyed on any context). The work ran on enwik8 slices, no enwik9, ≤2 concurrent encodes, and touched no shipped code.

The first result was a measurement finding that reshaped the method. The shipped `default_pipeline` runs `reorder`, whose greedy TF-IDF article ordering (`reorder.rs::block_tf`) uses a std `HashMap` with a per-process random seed, so the article permutation — and thus the encoded stream and its bpb — shifts run-to-run by roughly ±0.001. This is not a correctness bug: the decoder restores order by sorting on each article's in-content `<id>`, never by replaying the permutation, so round-trip is unaffected by which order the encoder chose (and the same-process round-trip tests could not have caught it regardless). But it means single-head marginals of ~0.0002–0.0007 cannot be ranked by comparing separate `cargo` runs. The fix was to measure a base spec and its candidate specs in one process on one permutation, so the difference is exact; `diversity_lab` was upgraded to take `LZR_HEADS_BASE` plus a `;`-separated candidate list. A daytime follow-up for CDR: a deterministic hasher in `block_tf` would make the shipped reorder permutation reproducible run-to-run (and might order slightly better or worse — untested).

With clean same-process measurement, the shipped 8-head stack (`1,w,2,s6,1:0.5,m:2,m2:2,mc:2`) gives −0.0272 over the headless full stack on the 8 MB [1M..9M] slice, and the diminishing-returns fear proved wrong: decorrelated *axes* keep paying about −0.0006 each. A digit-field head (`num`, exp 3) adds −0.0007; a secondary-match-prediction × previous-byte head (`m2c`, exp 6) adds another −0.0006 (a different axis from num, so it stacks cleanly); a sparse byte-context head over bytes-back {1,4} (`s9`) adds another −0.0006. The triple `num,m2c,s9` reaches −0.0016 to −0.0021 on 8 MB and confirms at −0.0018 on 20 MB [1M..21M] (with `num,m2c` at −0.0013 on both slices — flat across scale). What does *not* pay: `mm` (exp 5, joint primary×secondary match) overlaps `m2c` and adds only −0.0002; `s10` {2,4} actively hurts (+0.0003, it overlaps the existing sparse heads); order-4, a slow dual-rate `num`, and a slow `word` add ~0; a fourth sparse `s17` {1,5} adds only −0.0002 at 20 MB, not worth its ~64 MB table. And `m2c` wants the match-head learning rate 2, not 4 (lr 4 halves its gain to −0.0004) — consistent with the shipped match heads. So the lever is real but small and axis-limited: three genuinely decorrelated contexts (digit, secondary-match×prev, a fresh sparse mask), then it saturates into the ±~0.0004 permutation-noise floor.

Disposition: a shipping proposal (add the three heads to `models()`, `num`/`s9` at the context lr, `m2c` at the match lr) is left for CDR — it is a shipped-code change needing a full-enwik8 gate (heads have held at scale before, but confirm) and a RAM-headroom check (+~190 MB at HEAD_BITS 16; `num`'s digit-field context has <256 values and can use a smaller table). If it holds, it moves the standing best 1.1446 → ~1.1428, the first movement since the reorder ship, at L(D)≈0. The `diversity_lab` same-process upgrade is committed (test-only); the head additions are not made.

## 2026-06-30 — online word dictionary, fully decomposed: LRU eviction is poison, a frozen-dynamic dict ties the static dict to within 0.012 bpb, and that residual is purely word-selection (NEGATIVE for Hutter, keeper for a general build)

CDR pushed on whether the shipped, frequency-mined word dictionary (paid as 2× `L(D)`) could move online — built by encoder and decoder identically as they scan, so the cost of learning a word is paid once in 1× `L(C)`, as that word's first literal occurrence. The motivating economic trade: the static dict's `L(D)` is tiny (29,969 B → 0.00048 bpb), so the prize is small, but the principle (move corpus-specific data out of the binary) matters for a general-purpose compressor. Three variants were measured on the net-free linear path (`online_dict.rs`, `online_dict_lab`).

The first cut, an LRU-evicting online dict, was a large loss — 8 MB enwik8 slice, N=2000: 1.8237 bpb, +0.141 vs the no-dict (casefold-only) 1.6825, despite shrinking the stream more. CDR called the cause before the data did: reusing a code byte for a new word makes the symbol→meaning mapping non-stationary, and the context models cannot learn a moving target. Removing eviction entirely (uncapped, three-tier ~1.05M slots → zero reuse on the slice) recovered nearly all of it: 1.6866, but that only ties no-dict and sits +0.030 behind the static dict.

The decomposition that settled it was a three-way full-enwik8 sweep (no-dict 1.5391 bpb): static-freq (frequency-ordered, stable codes — the shipped form), static-appearance (the dynamic dict's appearance-ordered word *choice*, but with stable codes from byte 0), and dynamic-frozen (appearance-ordered words, online codes). Subtracting columns isolates the two costs. The online/non-stationarity cost (dynamic − static-appearance) collapses to ~0 once warmup amortizes over 100 MB instead of an 8 MB slice (≤ +0.0064 across all N, vs +0.005–0.012 on the slice) — the small-slice warmup, where the dictionary is still filling for a large fraction of the run, was inflating it. With non-stationarity gone, the entire dynamic-vs-static gap is word-selection (static-appearance − static-freq), and that shrinks with N as the appearance- and frequency-ordered word *sets* converge: +0.0222 at N=1000 → +0.0002 at N=32000. Both curves knee at N≈8000: static-freq best 1.4833 (−0.0558 vs no-dict), dynamic-frozen best 1.4954 (−0.0427), best-to-best +0.0121 bpb — all of it word-selection. (On the 8 MB slice the selection cost went *negative* at large N from the appearance list overfitting the test slice; that artifact vanishes on the full corpus.)

The verdict splits by product. For Hutter the static dict wins: 0.0005 bpb of `L(D)` buys ~0.012 bpb of `L(C)` — frequency foresight the causal decoder structurally cannot have (it cannot rank words it has not yet seen), plus a smaller frequency-curated optimal set. In retrospect this is obvious: the decoder cannot derive an ideal dictionary without the encoder shipping it, which is exactly what `L(D)` pays for. For a general-purpose build (which cannot ship a corpus dictionary) the frozen-dynamic dict is the right design — −0.0427 vs no-dict, ships nothing, within 0.012 of the curated static dict. The untested lever to close that residual at `L(D)≈0` is a frequency proxy without foresight: mint a word into the dict only after it has been seen ≥ K times, biasing selection toward frequency without the non-stationarity that promotion (changing an existing code) would reintroduce. This is one more instance of the running theme — offline foresight pays only as a global, shipped/amortized decision, never as something the causal decoder reconstructs for free. Kept as `online_dict` (test-only; `new` = LRU, `frozen` = fill-then-freeze) plus the `online_dict_lab` sweep.
